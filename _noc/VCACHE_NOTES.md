...
| DAT channel | 32 B/cyc (256 b flit, 2 flits/line) | **64 B/cyc native 512-bit DAT** (`vcache3d/rtl/top/hnf_vcache3d_adapter.v`), NoC flit geometry stays 256 b |
...
- **What was deliberately NOT changed in the NoC, but is now adapted at the cache edge:**
  - `CHIE_DATA_WIDTH_PARAM` stays 256. The DAT flit geometry (2 flits/line,
    `dataid` 00/10, `pe[1:0]`, DBF byte-merge) is built around it. Instead of
    changing the protocol, `vcache3d/rtl/top/hnf_vcache3d_adapter.v` packs two
    256-bit flits into a native 512-bit line access, so the cache array moves
    64 B/cycle while the NoC still uses 256-bit flits.
...
## 7. VCACHE-3D: the physical deliverables, built (August 2026)

Section 6 closed by listing what a 96 MiB profile does **not** give you:
hybrid-bonded SRAM, interconnect parasitics, ECC policy, frequency, thermals,
yield and measured PPA. Those are now in `_noc/vcache3d/` — roughly 46k lines
of RTL, generators, models, testbenches and physical-design collateral.

| gap from section 6 | where it is closed |
|---|---|
| hybrid-bonded stacked SRAM | `rtl/stack/` — subarray, 2048-macro bank array (generated), bond lane/CRC/channel/link, dielet top with no cache control logic on it |
| interconnect parasitics | `pd/scripts/timing_model.py` (M4/M8 ps/µm, 31 ps bond pad), `pd/package/` (1376 lane pairs, mirror-checked) |
| ECC policy | `rtl/ecc/` — SECDED(32/64/128/256/512) generated and self-proven, 4×SECDED(128,9) per line, pipelined decoder, scrubber, CE tracker, error log |
| frequency | 3.0 GHz base / 2.2 GHz macro-sliced cache, all paths meet; the model found six violations first and the RTL was restructured |
| thermals | `pd/thermal/thermal_model.py` — inverted Zen 5 stack, +0.5 °C penalty (was +3.3 °C), throttle at 16 W, 0.6 ms dielet time constant |
| yield | 99.94 % post-repair at D0 = 0.07 /cm², with spare rows/columns/lanes, MBIST, eFuse and a repair controller |
| measured PPA | `docs/PPA_REPORT.md`, regenerated by `make reports` |

Attach to the HN-F: `rtl/top/hnf_vcache3d_adapter.v` is a drop-in replacement
for `hnf/hnf_data_sram.v` (same index/ways/data port, plus stall and error
outputs). See `vcache3d/docs/INTEGRATION.md`.

Build from here:

```
make -C _noc vcache3d          # check generated sources, lint, models, reports
make -C _noc/vcache3d sim-all  # testbenches (needs VCS or Verilator)
```

Still outstanding, stated plainly: no simulator, synthesiser or PDK exists in
this environment, so the five testbenches have not been executed and the
OpenROAD flow in `pd/openroad/` has not been run. Everything that *can* run
without a tool — the linter, the golden models, the ECC self-tests, and all
five physical models — does run, and is what the numbers above come from.
