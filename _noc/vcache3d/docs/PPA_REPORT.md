...
Target: 3.0 GHz base tier, 1.5 GHz cache tier, SS corner (0.855 V logic,
0.68 V array, 125 C).

| tier | worst path | slack | Fmax |
|---|---|---|---|
| base | router FIFO -> request register -> 1.8 mm wire -> slice | +4.3 ps | 3.04 GHz |
| cache | subarray macro -> 16:1 sub mux -> bank output flop | +23.7 ps | 1.56 GHz |
...
| ECC decoder split into syndrome/correct stages | the flat decoder is 379 ps |
| registered bank output on the dielet | 465 ps access + 16:1 + 32:1 + 1.1 mm did not fit |
...
| registered bond TX after the CRC | CRC + steering + 900 um + pad was 383 ps |
...
times: tag 8192x39 = 175 ps, base data 8192x576 = 190 ps, stacked 1024x148 =
465 ps.
...
| stacked hit (ways 4-15) | 21 | 7.0 ns |
| miss (to memory) | 12 + DRAM | -- |

The stacked region costs +9 base cycles. AMD quotes +4 core cycles for
V-Cache; the difference is the deliberately slow 1.5 GHz dielet, which is what
buys 29 Mb/mm2 and 0.62 W. Two things make it a good trade: 24 MiB of the 96
lives on the base die at the fast latency (a monolithic V-Cache slice has no
such region), and 7 ns is still an order of magnitude better than the DRAM
access it replaces.
...
| block | area |
|---|---|
| stacked subarray macro (1024x148) | 0.0041 mm2 |
| macros per dielet | 2048 (4 quadrants x 32 banks x 16 subarrays) |
...
The dielet sits between the hot base die and the lid, so base-die heat passes
through it. Steady state for one 10.57 mm2 column, 45 C ambient, 12 W of core
power underneath:

| node | temperature |
|---|---|
| base-die junction | 86.8 C |
| dielet junction | 86.2 C |
| lid | 82.0 C |
| same column without a dielet | 83.5 C |
| **stacking penalty** | **+3.3 C** |
...
1.5 GHz operation, per-bank sleep, and a bond field spread along the die edge
(the bonds are also the best heat path between the dies).
...
| line | 64 B | 64 B |
| stacked latency adder | +4 core cycles | +9 base cycles |
| bandwidth | ~700 GB/s per CCD | 648 GB/s per package |
...
The two places this design is behind are stacked latency (+9 vs +4 cycles) and
the absence of silicon measurements. The two places it is ahead are the base-die
fast region and the fact that every number above is reproducible from a program
in this repository.
